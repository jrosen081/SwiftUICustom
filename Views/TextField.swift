...
public struct TextField<Label: View>: View {
	...
	public init<S>(_ title: S, text: Binding<String>) where S : StringProtocol, Label == Text {
		self.label = Text(String(title))
		self.binding = text
	}
	
	public init<S, T>(_ title: S, value: Binding<T>, formatter: Formatter) where S : StringProtocol, Label == Text {
		self.label = Text(String(title))
